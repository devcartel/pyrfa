...
void StandardOut::out(const DataBuffer& dataBuffer)
{
	...
	switch(dataBufferType)
	{
		...
		case DataBuffer::Int32Enum:
			// Need to make sure ...
			printf("%d",dataBuffer.getInt32());
			break;

		case DataBuffer::UInt32Enum:
			// Need to make sure ...
			printf("%u",dataBuffer.getUInt32());
			break;
		...
		case DataBuffer::StringRMTESEnum:
			strncpy(sData,(char*)dataBuffer.getBuffer().c_buf(),dataBuffer.getBuffer().size());
			sData[dataBuffer.getBuffer().size()] = '\0';
			printf("%s",sData);
			break;
		...
		case DataBuffer::UnknownDataBufferEnum:
			strcpy(sData,"UnknownDataBuffer");
			printf("%s",sData);
			break;
...
void StandardOut::out(const RespStatus& input)
{
	...
	switch (input.getDataState())
	{
		...
		case RespStatus::SuspectEnum:
			...
			break;
		case RespStatus::UnspecifiedEnum:
			_utilString = const_cast<char *>("Unspecified");
			break;
		default:
...
void StandardOut::out(const rfa::common::RFA_String & input)
{
