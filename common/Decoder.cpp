...
{	
	...
	else
	{
		...
		if (fieldDef)
		{
			if(fieldDef->getFieldId() == 0)
			{
				fieldName = fieldDef->getName();
				_out->outBegin(input, "Temporary Dictionary Change new DictionaryId:");
				printf("\t");
				decodeData(input.getData(DataBuffer::IntEnum));
			}
			else
			{
				//check if enum
...
void Decoder::decodeSeries(const Series& input)	
{
	...
	if (input.getHintMask() & Series::SummaryDataFlag)
		decodeData(input.getSummaryData());
...
void Decoder::decodeDataBuffer(const DataBuffer& dataBuffer)	
{
	...
	case DataBuffer::EnumerationEnum:
		if(_currentEnumDef)
		{
			_out->out(_currentEnumDef->findEnumVal(dataBuffer.getEnumeration()).c_str());
			char tmp[20];
